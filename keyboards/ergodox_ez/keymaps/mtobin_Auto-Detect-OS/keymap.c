...
enum custom_keycodes {
    ...
    QTPAIR,  // Quote pair '', ""
    CRTPAIR, // Caret pair <>
    USR_MOD, // Command on MacOS, Ctrl on Windows
    ALFRED
};

enum layers {
    _CMM,     // Colemak MacOS, default layer
    _QWERTYM, // QWERTY MacOS
    _CMW, // Colemak Windows
    _QWERTYW, // QWERTY Windows
    _FUNC,   // Function keys
    _NUM,    // NUM
};
...
enum tap_dance_codes {
    UP,       // Up, PgUp, Home
    DWN,      // Down, PgDn, End
    SHFT_PRN, // Left Paren on tap, Shift on hold, CapsLock on double tap
};

enum combo_events {
    EM_EMAIL,  // Types out my work email
    LYR_SWTCH, // Switch between Colemak and QWERTY layouts
    OS_SWTCH,  // Switch between MacOS and Windows layouts
    COMBO_LENGTH
};

uint16_t               COMBO_LEN     = COMBO_LENGTH; // remove the COMBO_COUNT define and use this instead!
const uint16_t PROGMEM email_combo[] = {KC_E, KC_M, COMBO_END};
const uint16_t PROGMEM lyr_swtch[]   = {KC_Q, KC_W, COMBO_END};
const uint16_t PROGMEM os_swtch[]    = {KC_M, KC_W, COMBO_END};

combo_t key_combos[] = {
  [EM_EMAIL] = COMBO_ACTION(email_combo),
  [LYR_SWTCH] = COMBO_ACTION(lyr_swtch),
  [OS_SWTCHJ] = COMBO_ACTION(os_swtch)
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
 /* _CMM:  Colemak DH Layer MacOS (Blue LED)
    *
    * ,--------------------------------------------------.           ,--------------------------------------------------.
    * |        |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
    * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
    ...
    */
    [_CMM] = LAYOUT_ergodox_pretty(
        ...
        KC_NO,          KC_LOPT, KC_LGUI, KC_LEFT, KC_RIGHT,                                             TD(UP), TD(DWN),  BRCPAIR, KC_RBRC,     KC_NO,
                                                         LGUI(KC_LALT), KC_NO,           KC_EQUAL, KC_MINUS,
                                                                        KC_NO,           KC_NO,
                                         LT(_NUM, KC_SPACE), KC_BSPACE, KC_NO,           KC_NO, LT(_FUNC, KC_DELETE), ENTER
        ),
        
    /* _QWERTYM: QWERTY MacOS Layer (Red LED)
    *
    * ,--------------------------------------------------.           ,--------------------------------------------------.
    * |        |      |      |      |      |      |      |           |      |      |      |      |      |      |   -    |
    * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
    ...
    *                               `---------------------'         `----------------------'
    */
    [_QWERTYM]  = LAYOUT_ergodox_pretty(
        KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO,    KC_NO,  KC_NO,               KC_NO, KC_NO, KC_NO,   KC_NO,    KC_NO,   KC_NO,     KC_NO,
...
/* _CMW:  Colemak DH Layer (Blue LED)
    *
    * ,--------------------------------------------------.           ,--------------------------------------------------.
    * |        |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
    * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
    ...
    *                               `---------------------'         `----------------------'
    */
    [_QWERTYW]  = LAYOUT_ergodox_pretty(
        KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO,    KC_NO,  KC_NO,               KC_NO, KC_NO, KC_NO,   KC_NO,    KC_NO,   KC_NO,     KC_NO,
        KC_TRNS, KC_Q,    KC_W,    KC_E,    KC_R,     T_TAB,  KC_NO,               KC_NO, KC_Y,  U_UNDLN, I_ITLCS,  O_OPEN,  P_PRINT,   KC_BSLASH,
        KC_TRNS, KC_A,    S_SAVE,  KC_D,    F_FIND,   KC_G,                               KC_H,  KC_J,    K_HPRLNK, KC_L,    KC_SCOLON, QTPAIR,
        KC_TRNS, Z_UNDO,  _X_CUT,   C_COPY, V_PASTE,  B_BOLD, KC_NO,               KC_NO, N_NEW, KC_M,    KC_COMMA, KC_DOT,  KC_SLASH,  KC_RSPC,
...
const uint8_t PROGMEM ledmap[][DRIVER_LED_TOTAL][5] = {
    ...
    [_CMM] =        {{131,255,255}, {131,255,255}, {131,255,255}, {131,255,255}, {131,255,255},         {131,255,255}, {131,255,255}, {131,255,255}, {131,255,255}, {131,255,255}, 
    ...
    // Cadmium red
    [_QWERTYM] =              {{0,245,245}, {0,245,245}, {0,245,245}, {0,245,245}, {0,245,245},         {0,245,245}, {0,245,245}, {0,245,245}, {0,245,245}, {0,245,245}, 
    ...
    // Electric Indigo
    [_CMW] =        {{188,255,255}, {188,255,255}, {188,255,255}, {188,255,255}, {188,255,255},         {188,255,255}, {188,255,255}, {188,255,255}, {188,255,255}, {188,255,255}, 
    ...
    // Bright Magenta
    [_QWERTYW] =    {{219,255,255}, {219,255,255}, {219,255,255}, {219,255,255}, {219,255,255},         {219,255,255}, {219,255,255}, {219,255,255}, {219,255,255}, {219,255,255}, 
...
void rgb_matrix_indicators_user(void) {
    ...
    switch (biton32(layer_state)) {
        case _CMM:
            set_layer_color(_CMM);
            break;
        case _QWERTM:
            set_layer_color(_QWERTYM);
            break;
        case _CMW:
            set_layer_color(_CMW);
            break;
        case _QWERTW:
            set_layer_color(_QWERTYW);
            break;
        case _FUNC:
...
void process_combo_event(uint16_t combo_index, bool pressed) {
    switch (combo_index) {
        ...
        case OS_SWTCH:
            if (pressed) {
                // Swap from MacOS QWERTY to Windows QWERTY
                if (layer_state & (1 << _QWERTYM)) {
                    layer_off(_QWERTYM);
                    layer_on(_QWERTYW);
                // Swap from Windows QWERTY to MacOS QWERTY
                } else if (layer_state & (1 << _QWERTYW)) {
                    layer_off(_QWERTYW);
                    layer_on(_QWERTYM);
                // Swap from MacOS CM to Windows CM
                } else if (layer_state & (1 << _CMM)) {
                    layer_off(_CMM);
                    layer_on(_CMW);
                // Swap from Windows CM to MacOS CM
                } else if (layer_state & (1 << _CMW)) {
                    layer_off(_CMW);
                    layer_on(_CMM);
                }
            }
